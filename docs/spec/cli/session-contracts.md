...
- `OPEN_TIMEOUT` 保留本次调用的根因，不从共享 WeappLog 推断或覆盖 code。
- 指定项目的 session 发现必须在探测/读取 session 文件前过滤 registry 中的其他项目；只有显式全局视图才遍历所有项目。未知项目时默认视图不读全局 session。
- 自动分配端口先用 TCP 连接检查已有 listener，再在非 WSL 宿主做 bind 检查；macOS 上通配地址 listener 与具体地址 bind 可能共存，不能只凭 bind 成功判断空闲，否则会误连其他项目。
...
- `open` / `await` / `doctor` 不自动读取共享 WeappLog；即使日志时间很近，也不能证明属于当前项目。原始错误只来自本次调用；显式 `devtools logs` 是不按项目隔离的诊断入口。
...
3. 无论 probe 成败，都不读取共享 WeappLog；仅使用本次 enable/probe 的诊断。
...
- `tests/diagnostic-privacy.test.cjs`：open/await/doctor 失败不读取共享日志，仍保留本次调用错误。
- `tests/port-allocation-privacy.test.cjs`：真实 wildcard listener 被排除；占用/失败探测不被当成空闲端口。
