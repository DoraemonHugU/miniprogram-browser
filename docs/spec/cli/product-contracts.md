...
自动诊断只使用本次 CLI 调用的原始错误、当前项目的 session 与 runtime 探测。`open`、`await`、`doctor` 不读取共享 `WeappLog`，不以其他项目或历史日志覆盖本次失败。只有显式 `devtools logs` 读取共享日志；该命令不承诺项目隔离，不可作为生产环境下的自动排障步骤。
...
本契约描述预期行为，不代表所有平台已经通过验收。构建、mock 测试、托管 CI 和真实 DevTools 门禁须分别报告；旧任务或历史通过记录不能代替当前候选版本的验证。
