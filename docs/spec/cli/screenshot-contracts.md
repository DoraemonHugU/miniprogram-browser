...
- 本契约记录各通道的依赖与失败边界、默认值和 snapshot 的显式视觉副作用。
...
// src/lib/visual.ts —— 结构图绘制不调官方截图，输入仍依赖渲染树与几何读取
...
| `layout` | 语义布局图 PNG | `captureLayoutScreenshot`（Jimp + canvas 字体） | 不调官方截图；仍依赖渲染树与几何读取 |
...
| 官方截图超时 | `captureScreenshotToPath` 抛超时错误；layout 不使用该截图通道，但渲染树或几何查询失败时也可能不可用 |
