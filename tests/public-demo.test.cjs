...
test('public demo interaction page covers scrolling, gestures, modal and transient state', () => {
  ...
  assert.match(wxml, /interaction-scroll[^>]*bindscroll="onScroll"/u)
  assert.match(wxml, /id="interaction-scroll-status">Container scrollTop: \{\{scrollTop\}\}/u)
  assert.match(wxml, /<swiper\b/u)
  assert.match(wxml, /interaction-swipe-target/u)
  assert.match(wxml, /bindtouchstart="onSwipeStart"/u)
  assert.match(wxml, /bindtouchend="onSwipeEnd"/u)
  assert.match(wxml, /id="interaction-swipe-status">Swipe: \{\{swipeStatus\}\}/u)
  assert.match(wxml, /bindlongpress=/u)
  assert.match(source, /wx\.showModal\s*\(/u)
  assert.match(source, /onPageScroll\s*\(/u)
  assert.match(source, /pageScrollTop/u)
  assert.match(source, /Math\.round\(event\.detail\.scrollTop\)/u)
  assert.match(source, /Math\.round\(event\.scrollTop\)/u)
  assert.match(source, /endX < this\.swipeStartX \? 'left' : 'right'/u)
  assert.match(source, /setTimeout\s*\(/u)
  assert.match(source, /\}, 2400\)/u)
  assert.match(wxml, /interaction-bottom/u)
  assert.match(wxml, /id="interaction-page-scroll-status">Page scrollTop: \{\{pageScrollTop\}\}/u)
})
