...
test('framework demos expose the same controls, repeated lists and navigation behaviors', () => {
  ...
  assert.match(taroSource, /id=['"]interaction-scroll['"][^>]*onScroll=/u)
  assert.match(taroSource, /id=['"]interaction-scroll-status['"][^>]*>Container scrollTop: \{scrollTop\}/u)
  assert.match(taroSource, /<Swiper\b/u)
  assert.match(taroSource, /interaction-swipe-target/u)
  assert.match(taroSource, /onTouchStart=/u)
  assert.match(taroSource, /onTouchEnd=/u)
  assert.match(taroSource, /id=['"]interaction-swipe-status['"][^>]*>Swipe: \{swipeStatus\}/u)
  assert.match(taroSource, /onLongPress=/u)
  assert.match(taroSource, /Taro\.showModal\s*\(/u)
  assert.match(taroSource, /usePageScroll/u)
  assert.match(taroSource, /id=['"]interaction-page-scroll-status['"][^>]*>Page scrollTop: \{pageScrollTop\}/u)
  assert.match(taroSource, /setScrollTop\(Math\.round\(/u)
  assert.match(taroSource, /setPageScrollTop\(Math\.round\(/u)
  assert.match(taroSource, /<Text[^>]*>Swipe: \{swipeStatus\}<\/Text>/u)
  assert.match(taroSource, /useState\('ready'\)/u)
  assert.match(taroSource, /touchX\(event\.changedTouches\) < swipeStartX\.current \? 'left' : 'right'/u)
  assert.match(taroSource, /2400/u)
  ...
  assert.match(uniSource, /id="interaction-scroll"[^>]*@scroll=/u)
  assert.match(uniSource, /id="interaction-scroll-status"[^>]*>Container scrollTop: \{\{ scrollTop \}\}/u)
  assert.match(uniSource, /<swiper\b/u)
  assert.match(uniSource, /interaction-swipe-target/u)
  assert.match(uniSource, /@touchstart=/u)
  assert.match(uniSource, /@touchend=/u)
  assert.match(uniSource, /id="interaction-swipe-status"[^>]*>Swipe: \{\{ swipeStatus \}\}/u)
  assert.match(uniSource, /@longpress=/u)
  assert.match(uniSource, /uni\.showModal\s*\(/u)
  assert.match(uniSource, /onPageScroll/u)
  assert.match(uniSource, /id="interaction-page-scroll-status"[^>]*>Page scrollTop: \{\{ pageScrollTop \}\}/u)
  assert.match(uniSource, /scrollTop\.value = Math\.round\(nextScrollTop\)/u)
  assert.match(uniSource, /pageScrollTop\.value = Math\.round\(nextScrollTop\)/u)
  assert.match(uniSource, /const swipeStatus = ref\("ready"\)/u)
  assert.match(uniSource, /endX < swipeStartX \? "left" : "right"/u)
  assert.match(uniSource, /2400/u)
