...
description: 当想要在微信开发者工具里用接近 agent-browser 的方式操作小程序时加载。适用于 snapshot/ref/click/fill/get/inspect/timeline 这类场景。
---
...
如果截图偶发超时，通常更像当前 session / DevTools 实例状态异常；优先 `close` 当前 session 后重新 `open`，必要时再重启 DevTools 实例。
