...
function normalizeProjectMapWindowsPrefix(prefix) {
  ...
  if (!/^[A-Z]:\\/u.test(normalized)) {
    throw new Error('Invalid project map. The Windows prefix must be a drive path like P:\\projects; use --project-map <linux=windows> or WECHAT_DEVTOOLS_PROJECT_MAP.')
  }
...
function parseProjectMapEntries(rawMap) {
  ...
  return chunks.map((chunk) => {
    ...
    return {
      linuxPrefix: normalizeProjectMapLinuxPrefix(chunk.slice(0, separatorIndex)),
      windowsPrefix: normalizeProjectMapWindowsPrefix(chunk.slice(separatorIndex + 1)),
    }
  }).sort((left, right) => right.linuxPrefix.length - left.linuxPrefix.length)
}
...
function matchesProjectMapPrefix(sourcePath, linuxPrefix) {
  ...
  return sourcePath === linuxPrefix || sourcePath.startsWith(`${linuxPrefix}/`)
}

function resolveMappedDevtoolsProjectPath(sourcePath, rawMap) {
  const normalizedSourcePath = String(sourcePath || '').trim().replace(/\\/gu, '/')
  if (!normalizedSourcePath || !rawMap) {
    return ''
  }

  const entries = parseProjectMapEntries(rawMap)
  const matchedEntry = entries.find((entry) => matchesProjectMapPrefix(normalizedSourcePath, entry.linuxPrefix))
  if (!matchedEntry) {
    return ''
  }

  const rest = matchedEntry.linuxPrefix === '/'
    ? normalizedSourcePath.slice(1)
    : normalizedSourcePath.slice(matchedEntry.linuxPrefix.length).replace(/^\/+/u, '')
  if (!rest) {
    return matchedEntry.windowsPrefix
  }

  const separator = matchedEntry.windowsPrefix.endsWith('\\') ? '' : '\\'
  return `${matchedEntry.windowsPrefix}${separator}${rest.replace(/\//gu, '\\')}`
}
...
function resolveDevtoolsProjectPath(config, options: AnyRecord = {}) {
  ...
  const hasWindowsBundle = String((config && config.cliPath) || '').toLowerCase().endsWith('.bat')
...
function buildAutomationArgs(config, options: AnyRecord = {}) {
  const hasWindowsBundle = String(config.cliPath || '').toLowerCase().endsWith('.bat')
...
function validateAutomationCliConfig(config) {
  ...
  const hasWindowsBundle = cliPath.toLowerCase().endsWith('.bat')
  if (hasWindowsBundle) {
    ...
    if (!existsSync(nodeExePath) || !existsSync(cliJsPath)) {
      throw new Error(`WeChat DevTools CLI bundle is incomplete near ${cliPath}; expected node.exe and cli.js next to cli.bat.`)
    }
    return
  }
...
function runDevtoolsCli(config, args, options: AnyRecord = {}) {
  validateAutomationCliConfig(config)
...
function runAutomationCli(config) {
  const { args } = buildAutomationArgs(config)
  return runDevtoolsCli(config, args)
}
...
function openDevtoolsProject(config, options: AnyRecord = {}) {
  ...
  const result = runDevtoolsCli(config, args, { timeoutMs: Number(options.timeoutMs || 30000) })
...
function resolveDevtoolsProjectPathForClose(config) {
  const explicitPath = String((config && config.devtoolsProjectPath) || '').trim()
  if (explicitPath) {
    return explicitPath
  }
...
function closeDevtoolsProject(config, options: AnyRecord = {}) {
  const projectPath = resolveDevtoolsProjectPathForClose(config)
  if (!projectPath) {
    return {
      attempted: false,
      ok: false,
  ...
  try {
    const result = runDevtoolsCli(config, args, { timeoutMs: options.timeoutMs || 30000 })
    const failure = parseAutomationCliFailure(result, config)
    if (failure) {
      return {
        attempted: true,
...
function enableAutomation(config) {
  const preOpen = openDevtoolsProject(config)
